...
struct PhysicsWorld {
  PhysicsWorld() {
    ...
    _dynamics_world = std::make_unique<btDiscreteDynamicsWorld>(
        _dispatcher.get(), _broad_phase.get(), _constraint_solver.get(), _collision_configuration.get()
    );
    _dynamics_world->setGravity(btVector3{0, -10, 0});
  }

  /** Returns the dynamics world. */
  btDynamicsWorld& dynamics_world() { return *_dynamics_world; }

  /** Returns the dynamics world. */
  const btDynamicsWorld& dynamics_world() const { return *_dynamics_world; }
