...
CharacterController::CharacterController(PhysicsEntity& phys_entity, const CCharacterController& component)
...
void on_character_controller_new(
  ...
  while (new_character_controller_channel.consume(subscriber_id, events, &num_events)) {
    ...
    for (int32_t i = 0; i < num_events; ++i) {
      ...
      physics_entity.character_controller =
          std::make_unique<CharacterController>(physics_entity, *components[i]);
      ...
      phys_data.phys_world.dynamics_world().addCollisionObject(
          &physics_entity.character_controller->ghost_object,
...
void on_character_controller_destroyed(
  ...
  while (destroyed_character_controller_channel.consume(subscriber_id, events, &num_events)) {
    for (int32_t i = 0; i < num_events; ++i) {
      ...
      phys_data.phys_world.dynamics_world().removeCollisionObject(
          &phys_entity->character_controller->ghost_object
      );
