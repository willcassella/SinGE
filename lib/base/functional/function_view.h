...
struct FunctionView<RetT(ArgTs...)> {
 ...
 public:
  ...
  FunctionView(RetT (&func)(ArgTs...)) {
    _data.func = &func;
    _func = [](UserData data, ArgTs... args) -> RetT { return data.func(std::forward<ArgTs>(args)...); };
  }

  /* Creates a FunctionView from a lambda/function pointer.
   * NOTE: This form allows lambdas and function pointers that do not exactly match the signature of this
   * FunctionView.*/
  template <typename Fn>
  ...
  /* You may not use a function reference that does not exactly match the signature of this FunctionView (use
   * a function pointer instead). */
  template <typename FRetT, typename... FArgTs>
  FunctionView(FRetT (&)(FArgTs...)) = delete;

  FunctionView(FunctionView& copy) : _data(copy._data), _func(copy._func) {}
  FunctionView(const FunctionView& copy) : _data(copy._data), _func(copy._func) {}
  FunctionView(FunctionView&& move) : _data(move._data), _func(move._func) {}
  FunctionView(const FunctionView&& move) : _data(move._data), _func(move._func) {}

  RetT invoke(ArgTs... args) const { return _func(_data, std::forward<ArgTs>(args)...); }

  RetT operator()(ArgTs... args) const { return _func(_data, std::forward<ArgTs>(args)...); }
