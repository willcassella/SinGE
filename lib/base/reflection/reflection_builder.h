...
namespace sge
{
    ...
    auto function_wrapper(tmp::list<ArgT, ArgTs...>, Fn&& fn, const ArgAny* args, const ConvertedArgs& ... realArgs)
    {
    ...
    void constructor_wrapper(void *addr, const ArgAny *args)
    {
        function_wrapper(
            tmp::list<ArgTs...>{},
            [addr](auto... args) { new(addr) T(args...); },
            args
        );
    }
    ...
    void constructor_wrapper(T(*constructor)(ArgTs...), void* addr, const ArgAny* args)
    {
        function_wrapper(
            tmp::list<ArgTs...>{},
            [addr, constructor](auto... args) { new(addr) T(constructor(args...)); },
            args
        );
    }
    ...
    struct NativeTypeInfoBuilder
    {
        ...
        NativeTypeInfoBuilder&& constructor()
        {
            ...
            type_data.constructors.insert(std::make_pair(sizeof...(ArgTs), NativeConstructorInfo{std::move(baseCData), std::move(cData)}));
            return std::move(*this);
        }
