...
NativeTypeInfo build_primitive_type_info(std::string name) {
  ...
  type_data.init = [](void* addr) { *static_cast<T*>(addr) = 0; };
  type_data.copy_init = [](void* addr, const void* copy) {
    *static_cast<T*>(addr) = *static_cast<const T*>(copy);
  };
  type_data.move_init = [](void* addr, void* move) { *static_cast<T*>(addr) = *static_cast<T*>(move); };
...
const NativeTypeInfo GetType<std::string>::type_info =
    NativeTypeInfoBuilder<std::string>("sge::String").flags(TF_RECURSE_TERMINAL);
}  // namespace specialized
}  // namespace sge
