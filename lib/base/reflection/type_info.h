...
struct SGE_BASE_EXPORT TypeInfo {
  ...
  explicit TypeInfo(Data data) : _data(std::move(data)) {
    // Generate hash code
   ...
   */
  TypeFlags_t flags() const { return _data.flags; }
   ...
   */
  const TypeInfo* base() const { return _data.base; }
   ...
   */
  bool is_native() const { return (_data.flags & TF_NATIVE) != 0; }

  /**
   * \brief Returns whether this type is a native (C++) primitive type.
   */
  bool is_primitive() const { return (_data.flags & TF_PRIMITIVE) != 0; }
  ...
  /**
   * \brief Returns the number of constructors for this type. This will match the number of calls to
   * 'enumerator' within 'enumerate_constructors'.
   */
   ...
   * \param enumerator A function to call with the arity and information for the constructor at each
   * iteration.
   */
  virtual void enumerate_constructors(FunctionView<ConstructorEnumeratorFn> enumerator) const = 0;

  /**
   * \brief Returns the number of named constructors for this type. This will match the number of calls to
   * 'enumerator' within 'enumerate_constructors'.
   */
   ...
   * In general you should use the global 'get_vtable' function, as it will search implementations on the
   * type, as well as on the interface. \param interf The interface to find the vtable for. \return A pointer
   * to the vtable object, which may be null if this type has not implemented the interface.
   */
  ...
  friend bool operator==(const TypeInfo& lhs, const TypeInfo& rhs) {
    return &lhs == &rhs || (lhs._data.native_type_info && rhs._data.native_type_info &&
                            *lhs._data.native_type_info == *rhs._data.native_type_info);
  }
