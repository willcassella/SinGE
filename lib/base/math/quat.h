...
struct SGE_BASE_EXPORT Quat {
  ...
  constexpr Quat() : _values{0, 0, 0, 1} {}

  Quat(float x, float y, float z, float w) : _values{x, y, z, w} {}
  ...
  float x() const { return _values[0]; }
  void x(float x) { _values[0] = x; }
  float y() const { return _values[1]; }
  void y(float y) { _values[1] = y; }
  float z() const { return _values[2]; }
  void z(float z) { _values[2] = z; }
  float w() const { return _values[3]; }
