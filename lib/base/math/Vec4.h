...
struct SGE_BASE_EXPORT Vec4 {
  ...
  constexpr Vec4() : _values{0, 0, 0, 0} {}
  constexpr Vec4(float x, float y, float z, float w) : _values{x, y, z, w} {}
  constexpr Vec4(Vec3 xyz, float w) : _values{xyz.x(), xyz.y(), xyz.z(), w} {}
  constexpr Vec4(float x, Vec3 yzw) : _values{x, yzw.x(), yzw.y(), yzw.z()} {}
  constexpr Vec4(Vec2 xy, Vec2 zw) : _values{xy.x(), xy.y(), zw.x(), zw.y()} {}
  constexpr Vec4(Vec2 xy, float z, float w) : _values{xy.x(), xy.y(), z, w} {}
  constexpr Vec4(float x, Vec2 yz, float w) : _values{x, yz.x(), yz.y(), w} {}
  constexpr Vec4(float x, float y, Vec2 zw) : _values{x, y, zw.x(), zw.y()} {}
  ...
  constexpr float w() const { return _values[3]; }
  void w(float w) { _values[3] = w; }
  ...
  std::string to_string() const { return format("<@, @, @, @>", x(), y(), z(), w()); }

  /* Serializes the state of this Vec4 to an archive. */
  void to_archive(ArchiveWriter& writer) const { writer.typed_array(_values, 4); }

  /* Deserializes the state of this Vec4 from an archive. */
  void from_archive(ArchiveReader& reader) { reader.typed_array(_values, 4); }
