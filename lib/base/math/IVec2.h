...
struct IVec2 {
  ...
  friend IVec2 operator*(float lhs, const IVec2& rhs) { return rhs * lhs; }
  friend bool operator==(const IVec2& lhs, const IVec2& rhs) {
    return lhs.x() == rhs.x() && lhs.y() == rhs.y();
  }
  friend bool operator!=(const IVec2& lhs, const IVec2& rhs) { return !(lhs == rhs); }
...
template <typename T>
SGE_REFLECT_TYPE_TEMPLATE(IVec2, T).template implements<IToArchive>().template implements<IFromArchive>();
