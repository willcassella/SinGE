...
class BinaryArchiveWriter final : public ArchiveWriter {
 ...
 public:
  BinaryArchiveWriter(std::vector<uint8_t>& buffer) : buffer(&buffer) {
  ...
  void pop() override {
    ...
    if (current_node.node_type == BAN_ARRAY_GENERIC || current_node.node_type == BAN_OBJECT) {
      ...
      *reinterpret_cast<BinaryArchiveSize_t*>(
          buffer->data() + current_node.offset + 1 + sizeof(BinaryArchiveSize_t)
      ) = span;
    }
  ...
  void null() override { node_set_type(BAN_NULL); }
  ...
  void string(const char* str, size_t len) override {
    ...
    buffer->insert(
        buffer->end(), reinterpret_cast<const uint8_t*>(str), reinterpret_cast<const uint8_t*>(str + len)
    );
  }

  void typed_array(const bool* arr, size_t size) override {
    // You may not change a node's type, or set it twice. Generic arrays are considered a different type than
    // typed arrays.
  ...
  void typed_array(const int8_t* arr, size_t size) override { impl_typed_array(BAN_ARRAY_INT8, arr, size); }

  void typed_array(const uint8_t* arr, size_t size) override { impl_typed_array(BAN_ARRAY_UINT8, arr, size); }

  void typed_array(const int16_t* arr, size_t size) override { impl_typed_array(BAN_ARRAY_INT16, arr, size); }

  void typed_array(const uint16_t* arr, size_t size) override {
    impl_typed_array(BAN_ARRAY_UINT16, arr, size);
  }

  void typed_array(const int32_t* arr, size_t size) override { impl_typed_array(BAN_ARRAY_INT32, arr, size); }

  void typed_array(const uint32_t* arr, size_t size) override {
    impl_typed_array(BAN_ARRAY_UINT32, arr, size);
  }

  void typed_array(const int64_t* arr, size_t size) override { impl_typed_array(BAN_ARRAY_INT64, arr, size); }

  void typed_array(const uint64_t* arr, size_t size) override {
    impl_typed_array(BAN_ARRAY_UINT64, arr, size);
  }

  void typed_array(const float* arr, size_t size) override { impl_typed_array(BAN_ARRAY_FLOAT, arr, size); }

  void typed_array(const double* arr, size_t size) override { impl_typed_array(BAN_ARRAY_DOUBLE, arr, size); }

  void push_array_element() override {
    assert(
        current_node.node_type == BAN_NULL || current_node.node_type == BAN_ARRAY_GENERIC
        /*You may not change a node's type. Generic arrays are considered a different type than typed
           arrays.*/
    );
  ...
  void impl_typed_array(BinaryArchiveNode array_type, const T* arr, size_t size) {
    ...
    // Push the array into the buffer
    buffer->insert(
        buffer->end(), reinterpret_cast<const uint8_t*>(arr), reinterpret_cast<const uint8_t*>(arr + size)
    );
  }
