 ...
 */
SGE_RESOURCE_API void
save_rgbf(const float* image, int32_t width, int32_t height, uint8_t num_channels, const char* path);
 ...
 */
SGE_RESOURCE_API void
dilate_rgbf(const float* image, int32_t width, int32_t height, uint8_t num_channels, float* out);
 ...
 * \param height The height of the image (pixels).
 * \param num_channels The number of channels in each pixel.
 * \param out A buffer to store the result of the smooth filter. This must be the same size as the given
 * image.
 */
SGE_RESOURCE_API void
smooth_rgbf(const float* image, int32_t width, int32_t height, uint8_t num_channels, float* out);
}  // namespace image_ops
}  // namespace sge
