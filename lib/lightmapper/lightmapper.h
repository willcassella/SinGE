 ...
 */
SGE_LIGHTMAPPER_API LightmapScene*
new_lightmap_scene(const LightmapOccluder* occluders, size_t num_occluders);

/**
 * \brief Frees resources associated with the given lightmap scene.
 * \param sene The scene to free.
 */
SGE_LIGHTMAPPER_API void free_lightmap_scene(LightmapScene* sene);

/**
 * \brief Computes a fudgy ambient term. Useful for scenes where direct lighting is not the sole exterior
 * contribution of light. \param ambient The amount of ambient light. \param width The width (in pixels) of
 * the lightmap. \param height The height (in pixels) of the lightmap. \param texel_mask The array of texel
 * masks in the lightmap. \param out_irradiance The pixels to assign the ambient radiance to.
 */
 ...
 */
SGE_LIGHTMAPPER_API void
lightmap_postprocess(int32_t width, int32_t height, int32_t num_steps, color::RGBF32* lightmap);
}  // namespace sge
