...
static Vec3 get_barycentric(const IVec2<int32_t>* tri, IVec2<int32_t> point) {
  ...
  const float u = float(
                      (tri[1].y() - tri[2].y()) * (point.x() - tri[2].x()) +
  ...
  const float v = float(
                      (tri[2].y() - tri[0].y()) * (point.x() - tri[2].x()) +
...
static bool within_triangle(const Vec3& bary) {
  ...
  return bary.x() >= -0.0001f && bary.x() <= 1.0001f && bary.y() >= -0.0001f && bary.y() <= 1.0001f &&
         bary.z() >= -0.0001f && bary.z() <= 1.0001f;
}
...
LightmapScene* new_lightmap_scene(const LightmapOccluder* occluders, size_t num_occluders) {
  // Create the scene
  auto* scene = new LightmapScene{};
  scene->rtc_device = rtcNewDevice();
  scene->rtc_scene = rtcDeviceNewScene(
      scene->rtc_device, RTC_SCENE_STATIC, RTC_INTERPOLATE | RTC_INTERSECT1 | RTC_INTERSECT8
  );
  ...
  for (size_t occluder_i = 0; occluder_i < num_occluders; ++occluder_i) {
    ...
    auto rtc_mesh = rtcNewTriangleMesh(
        rtc_scene, RTC_GEOMETRY_STATIC, occluder_mesh->num_triangles(), occluder_mesh->num_verts()
    );
...
void free_lightmap_scene(LightmapScene* scene) {
  ...
  for (size_t mesh_i = 0; mesh_i < num_objects; ++mesh_i) {
    ...
    for (size_t tri_i = 0; tri_i < num_triangles; ++tri_i) {
      ...
      for (int vert_i = 0; vert_i < 3; ++vert_i) {
        ...
        // Set the normal
        vert_norms[vert_i] = (mesh_inverse_transpose *
                              Vec3{
        ...
        // Set the tangent
        vert_tans[vert_i] = (mesh_inverse_transpose *
                             Vec3{
        ...
        vert_tans[vert_i] =
            (vert_tans[vert_i] - Vec3::dot(vert_tans[vert_i], vert_norms[vert_i]) * vert_norms[vert_i])
                .normalized();

        // Calculate bitangent
        vert_btans[vert_i] = Vec3::cross(vert_norms[vert_i], vert_tans[vert_i]) *
                             triangle_bitangent_signs[vert_indices[vert_i]];
      }
...
void compute_indirect_radiance(
  ...
  const float normalization_factor =
      1.f / (static_cast<float>(num_sample_sets * num_accumulations) * RAY_SET_SIZE) * 2;
    ...
    for (int32_t x = 0; x < width; ++x) {
      ...
      for (int iter_i = 0; iter_i < num_sample_sets; ++iter_i) {
        ...
        for (int i = 0; i < RAY_SET_SIZE; ++i) {
          ...
          // Set direction
          const Vec3 dir =
              texel_TBN * Vec3{randf(-1.0f, 1.0f), randf(-1.0f, 1.0f), randf(0.15f, 1.0f)}.normalized();
          ray_dirs[i] = dir;
        ...
        for (int i = 0; i < RAY_SET_SIZE; ++i) {
          ...
          const auto hit_irradiance =
              hit_mesh.irradiance[hit_irradiance_uv.x() + hit_irradiance_uv.y() * hit_mesh.irradiance_width];
        ...
        // Write normal irradiance, dividing by normalization factor (Monte Carlo integration), and factoring
        // in base color
...
void lightmap_postprocess(int32_t width, int32_t height, int32_t num_steps, color::RGBF32* lightmap) {
