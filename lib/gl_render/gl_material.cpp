...
void get_material_standard_uniforms(GLuint mat_id, MaterialStandardUniforms* out_uniforms) {
  ...
  out_uniforms->lightmap_direct_mask_uniform =
      glGetUniformLocation(mat_id, LIGHTMAP_DIRECT_MASK_UNIFORM_NAME);
...
void init_material_lightmap_params(const GLuint mat_id, const MaterialStandardUniforms uniforms) {
  ...
  glProgramUniform1i(
      mat_id, uniforms.lightmap_direct_mask_uniform, LIGHTMAP_DIRECT_MASK_TEXTURE_SLOT - GL_TEXTURE0
  );
}
