...
static void render_spotlight_shadowmaps(const RenderScene_Commands& commands) {
  glDepthMask(GL_TRUE);
  glCullFace(GL_FRONT);
  ...
  for (const auto& spotlight : commands.spotlights) {
    RenderCommand_bind_framebuffer(
        spotlight.shadow_framebuffer, spotlight.shadow_width, spotlight.shadow_height
    );
...
RenderScene_Lightmap get_lightmap(const RenderScene_Commands& commands, const NodeId node_id) {
...
void RenderScene_insert_static_mesh_commands(
  ...
  for (size_t i = 0; i < num_nodes; ++i) {
    ...
    const auto& mesh_resource =
        RenderResource_get_static_mesh_resource(resources, static_mesh->mesh().c_str());

    // Get the material resource for this instance
    const auto& material_resource =
        RenderResource_get_material_resource(resources, static_mesh->material().c_str());
    ...
    if (material_iter == commands.standard_path_material_indices.end()) {
      commands.standard_path_material_indices.insert(
          std::make_pair(material_resource.program_id, commands.standard_path_material_instances.size())
      );
    ...
    if (mesh_iter == material.mesh_indices.end()) {
      material.mesh_indices.insert(std::make_pair(mesh_resource.vao, material.mesh_instances.size()));
...
void RenderScene_insert_spotlight_commands(
  ...
  for (size_t i = 0; i < num_spotlights; ++i) {
    ...
    glVertexAttribPointer(
        gl_material::POSITION_ATTRIB_LOCATION, 3, GL_FLOAT, GL_FALSE, sizeof(Vec3), nullptr
    );
    ...
    glVertexAttribPointer(
        gl_material::MATERIAL_TEXCOORD_ATTRIB_LOCATION, 2, GL_FLOAT, GL_FALSE, sizeof(Vec2), nullptr
    );
...
void RenderScene_clear(RenderScene_Commands& commands) {
  ...
  for (auto node : commands.node_lightmaps) {
    GLuint textures[] = {
        node.second.x_basis_tex, node.second.y_basis_tex, node.second.z_basis_tex, node.second.direct_mask_tex
    };
