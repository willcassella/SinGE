...
static void on_node_transform_update(
  ...
  while (node_transform_update_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_update_matrices(commands, nodes_ids, world_transforms, num_events);
  }
}
...
static void on_static_mesh_new(
  ...
  while (new_static_mesh_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_insert_static_mesh_commands(commands, resources, nodes, components, num_events);
  }
}
...
static void on_static_mesh_destroy(
  ...
  while (destroyed_static_mesh_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_remove_static_mesh_commands(commands, node_ids, num_events);
  }
}
...
static void on_spotlight_new(
  ...
  while (new_spotlight_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_insert_spotlight_commands(commands, resources, nodes, components, num_events);
  }
}
...
static void on_spotlight_modified(
  ...
  while (modified_spotlight_event_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_update_spotlight_commands(commands, resources, nodes, components, num_events);
  }
}
...
static void on_spotlight_destroy(
  ...
  while (destroyed_spotlight_event_channel.consume(subscriber_id, events, &num_events)) {
    ...
    // Update render scene
    RenderScene_remove_spotlight_commands(commands, node_ids, num_events);
  }
}

static void initialize_render_scene(RenderScene_Commands& render_scene, Scene& scene) {
...
static void attach_framebuffer_layer(GLuint layer, GLenum attachment) {
  glFramebufferTexture2D(GL_FRAMEBUFFER, attachment, GL_TEXTURE_2D, layer, 0);
}

static GLuint create_viewport_program(GLuint v_shader, GLuint f_shader) {
  ...
  glBindAttribLocation(
      program, gl_material::MATERIAL_TEXCOORD_ATTRIB_LOCATION, gl_material::MATERIAL_TEXCOORD_ATTRIB_NAME
  );
...
GLRenderSystem::GLRenderSystem(const Config& config) {
  ...
  _state->resources.missing_mesh =
      RenderResource_get_static_mesh_resource(_state->resources, config.missing_mesh.c_str());

  // Create the default material resource
  _state->resources.missing_material =
      RenderResource_get_material_resource(_state->resources, config.missing_material.c_str());
  ...
  attach_framebuffer_layer(
      _state->gbuffer_layers[GBufferLayer::DEPTH_STENCIL], GBUFFER_DEPTH_STENCIL_ATTACHMENT
  );
  ...
  set_render_target_params();
  attach_framebuffer_layer(_state->gbuffer_layers[GBufferLayer::POSITION], GBUFFER_POSITION_ATTACHMENT);
  ...
  set_render_target_params();
  attach_framebuffer_layer(_state->gbuffer_layers[GBufferLayer::NORMAL], GBUFFER_NORMAL_ATTACHMENT);
  ...
  set_render_target_params();
  attach_framebuffer_layer(_state->gbuffer_layers[GBufferLayer::ALBEDO], GBUFFER_ALBEDO_ATTACHMENT);
  ...
  set_render_target_params();
  attach_framebuffer_layer(
      _state->gbuffer_layers[GBufferLayer::ROUGHNESS_METALLIC], GBUFFER_ROUGHNESS_METALLIC_ATTACHMENT
  );
  ...
  set_render_target_params();
  attach_framebuffer_layer(_state->gbuffer_layers[GBufferLayer::IRRADIANCE], GBUFFER_IRRADIANCE_ATTACHMENT);
  ...
  set_render_target_params();
  attach_framebuffer_layer(_state->post_buffer_hdr, POST_BUFFER_HDR_ATTACHMENT);
  ...
  constexpr float QUAD_VERTEX_DATA[] = {
      ...
      -1.f,
      0.f,
      0.f,  // Bottom-left point
      1.f,
      -1.f,
      1.f,
      0.f,  // Bottom-right point
      1.f,
      1.f,
      1.f,
      1.f,  // Top-right point
  ...
  glVertexAttribPointer(
      gl_material::POSITION_ATTRIB_LOCATION, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 4, nullptr
  );
  glVertexAttribPointer(
  ...
  _state->scene_program_light_intensity_uniform =
      glGetUniformLocation(_state->scene_shader_program, "light_intensity");
  ...
  glProgramUniform1i(
      _state->post_shader_program, glGetUniformLocation(_state->post_shader_program, "hdr_buffer"), 5
  );
  _state->post_program_gamma_uniform = glGetUniformLocation(_state->post_shader_program, "gamma");
  _state->post_program_brightness_uniform =
      glGetUniformLocation(_state->post_shader_program, "brightness_boost");
  ...
  glVertexAttribPointer(
      DEBUG_LINE_POSITION_ATTRIB_LOCATION, 3, GL_FLOAT, GL_FALSE, sizeof(DebugLineVert), nullptr
  );
  glVertexAttribPointer(
  ...
  // Bind vertex attributes
  glBindAttribLocation(
      _state->debug_line_program, DEBUG_LINE_POSITION_ATTRIB_LOCATION, DEBUG_LINE_POSITION_ATTRIB_NAME
  );
  glBindAttribLocation(
      _state->debug_line_program, DEBUG_LINE_COLOR_ATTRIB_LOCATION, DEBUG_LINE_COLOR_ATTRIB_NAME
  );
...
void GLRenderSystem::set_viewport(int width, int height) {
  ...
  glBindTexture(GL_TEXTURE_2D, _state->gbuffer_layers[GBufferLayer::NORMAL]);
  upload_render_target_data(
      width, height, GBUFFER_NORMAL_INTERNAL_FORMAT, GBUFFER_NORMAL_UPLOAD_FORMAT, GBUFFER_NORMAL_UPLOAD_TYPE
  );

  // Resize albedo layer
  glBindTexture(GL_TEXTURE_2D, _state->gbuffer_layers[GBufferLayer::ALBEDO]);
  upload_render_target_data(
      width, height, GBUFFER_ALBEDO_INTERNAL_FORMAT, GBUFFER_ALBEDO_UPLOAD_FORMAT, GBUFFER_ALBEDO_UPLOAD_TYPE
  );
...
void GLRenderSystem::render_scene(Scene& scene, SystemFrame& /*frame*/) {
  ...
  // Consume events
  on_static_mesh_new(
      scene, *_new_static_mesh_channel, _new_static_mesh_sid, _state->resources, _state->render_scene
  );
  on_static_mesh_destroy(*_destroyed_static_mesh_channel, _destroyed_static_mesh_sid, _state->render_scene);
  on_spotlight_new(
      scene, *_new_spotlight_channel, _new_spotlight_sid, _state->resources, _state->render_scene
  );
  on_spotlight_modified(
      scene, *_modified_spotlight_channel, _modified_spotlight_sid, _state->resources, _state->render_scene
  );
  on_spotlight_destroy(*_destroyed_spotlight_channel, _destroyed_spotlight_sid, _state->render_scene);
  on_node_transform_update(
      *_modified_node_transform_channel, _modified_node_transform_sid, _state->render_scene
  );
  ...
  std::array<GLenum, 5> draw_buffers = {GL_NONE, GL_NONE, GL_NONE, GL_NONE, GBUFFER_IRRADIANCE_ATTACHMENT};
  ...
  glProgramUniform1f(
      _state->post_shader_program, _state->post_program_gamma_uniform, scene.get_raw_scene_data().scene_gamma
  );
  glProgramUniform1f(
      _state->post_shader_program,
      _state->post_program_brightness_uniform,
      scene.get_raw_scene_data().scene_brightness_boost
  );
