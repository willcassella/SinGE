...
void RenderCommand_bind_framebuffer(const GLuint framebuffer, const GLuint width, const GLuint height);
...
/**
 * \brief Renders the given meshes to the currently bound framebuffer with the currently bound material and
 * parameters. \param uniforms Uniform locations for the currently bound material. \param mesh The mesh to
 * render. \param mesh_instances The instances of this mesh to render. \param num_instances The number of mesh
 * instances to render.
 */
...
/**
 * \brief Command to render sets of lines to the currently bound framebuffer with the currently bound material
 * and parameters. \param lines Sets of lines to draw. NOTE: You can have arbitrarily many lines within the
 * buffer for each 'RenderCommand_Lines' object, which is perferable over multiple commnands. \param num_lines
 * The number of lines to draw.
 */
void RenderCommand_render_lines(const RenderCommand_Lines* lines, size_t num_lines);

void render_scene_shade_hdr(GLuint framebuffer, const GLRenderSystem::State& render_scene, Mat4 view);
}  // namespace gl_render
}  // namespace sge
