...
SGE_REFLECT_TYPE(sge::SceneLightmap).implements<IToArchive>().implements<IFromArchive>();
...
void SceneLightmap::to_archive(ArchiveWriter& writer) const {
  ...
  for (const auto& element : lightmap_elements) {
    ...
    image_ops::save_rgbf_to_memory(
        element.second.basis_x_radiance.data()->vec(), width, height, 3, &image_buff, &image_buff_size
    );
    ...
    image_ops::save_rgbf_to_memory(
        element.second.basis_y_radiance.data()->vec(), width, height, 3, &image_buff, &image_buff_size
    );
    ...
    image_ops::save_rgbf_to_memory(
        element.second.basis_z_radiance.data()->vec(), width, height, 3, &image_buff, &image_buff_size
    );
...
void SceneLightmap::from_archive(ArchiveReader& reader) {
  ...
  reader.enumerate_object_members([this, &reader](const char* node_id_str) {
    ...
    // Construct image
    image_ops::load_rgbf_from_memory(
        compressed_buff.data(),
