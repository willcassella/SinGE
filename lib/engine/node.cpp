...
SGE_REFLECT_TYPE(sge::NodeId).flags(TF_RECURSE_TERMINAL).property("is_null", &NodeId::is_null, nullptr);
...
Vec3 Node::get_pending_local_position() const {
  ...
  const auto& trans_mod =
      _scene->get_raw_scene_data().system_node_local_transform_changes[_transform_mod_index];
