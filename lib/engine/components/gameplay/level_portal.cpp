...
struct CLevelPortal::SharedData : CSharedData<CLevelPortal> {
  SharedData() : change_level_channel(sizeof(EChangeLevel), 1) {}
...
CLevelPortal::CLevelPortal(NodeId node_id, SharedData& shared_data)
    : _node_id(node_id), _shared_data(&shared_data) {}

void CLevelPortal::register_type(Scene& scene) {
  scene.register_component_type(
      type_info, std::make_unique<BasicComponentContainer<CLevelPortal, SharedData>>()
  );
}
