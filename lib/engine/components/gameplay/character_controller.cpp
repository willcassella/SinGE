...
struct CCharacterController::SharedData : CSharedData<CCharacterController> {
  SharedData()
      : jump_channel(sizeof(EJump), 2), turn_channel(sizeof(ETurn), 2), walk_channel(sizeof(EWalk), 2) {}
...
void CCharacterController::register_type(Scene& scene) {
  scene.register_component_type(
      type_info, std::make_unique<BasicComponentContainer<CCharacterController, SharedData>>()
  );
}
