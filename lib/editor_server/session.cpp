...
void Session::async_send_message() {
  ...
  asio::async_write(
      socket,
      asio::buffer(_outgoing_packets.front(), _outgoing_packets.front()->packet_size()),
...
void Session::async_receive_message() {
  asio::async_read(
      ...
      [self = shared_from_this()](const std::error_code& error, size_t /*len*/) {
        ...
        std::cout << "Completed query at " << std::chrono::system_clock::to_time_t(completed_time)
                  << std::endl;
        std::cout << std::endl;
