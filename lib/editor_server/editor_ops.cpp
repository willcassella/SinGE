...
// #include "lib/lightmapper/lightmapper.h"
...
static void read_type_info(const TypeInfo& type, ArchiveWriter& writer) {
  ...
  type.enumerate_properties([&writer](const char* propName, const PropertyInfo& propInfo) {
    ...
    if (propInfo.is_read_only() || propInfo.flags() & PF_EDITOR_HIDDEN ||
        propInfo.type().flags() & TF_CONTAINER) {
      return;
    }
...
void destroy_component_query(Scene& scene, ArchiveReader& reader, ArchiveWriter& writer) {
  ...
  reader.enumerate_object_members([&scene, &reader, &writer](const char* component_type_name) {
    ...
    reader.enumerate_array_elements([component_type_name, container, &scene, &reader, &writer](size_t /*i*/) {
      ...
      std::cout << "Destroyed '" << component_type_name << "' component on node " << node.to_u64()
                << std::endl;
...
void component_property_update_query(Scene& scene, ArchiveReader& reader, ArchiveWriter& writer) {
  ...
  reader.enumerate_object_members([&scene, &reader, &writer](const char* component_type_name) {
    ...
    reader.enumerate_object_members([type, container, &scene, &reader, &writer](const char* node_id_str) {
      ...
      // Deserialize it
      std::cout << "Writing properties of '" << type->name() << "' component on node '" << node_id_str << "'"
                << std::endl;
      writer.push_object_member(node_id_str);
...
/*             lightmap_element.basis_x_radiance.assign(lightmap_element.width * lightmap_element.height,
 * color::RGBF32::black()); */
/*             lightmap_element.basis_y_radiance.assign(lightmap_element.width * lightmap_element.height,
 * color::RGBF32::black()); */
/*             lightmap_element.basis_z_radiance.assign(lightmap_element.width * lightmap_element.height,
 * color::RGBF32::black()); */
...
/*             lightmap.lightmap_texels = (LightmapTexel*)calloc(lightmap.width * lightmap.height,
 * sizeof(LightmapTexel)); */
/*             lightmap.lightmap_texel_mask = (uint8_t*)calloc(lightmap.width * lightmap.height, 1); */
/*             lightmap.irradiance_front = (color::RGBF32*)calloc(lightmap.width * lightmap.height,
 * sizeof(color::RGBF32)); */
/*             lightmap.irradiance_back = (color::RGBF32*)calloc(lightmap.width * lightmap.height,
 * sizeof(color::RGBF32)); */
...
/*     std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(scene_construct_end -
 * scene_construct_start).count(); */
/*     std::cout << " milliseconds." << std::endl; */
...
/*     std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(gen_tex_end -
 * gen_tex_start).count(); */
/*     std::cout << " milliseconds." << std::endl; */
...
/*             // Compute mask value */
/*             const auto mask = irradiance_value.red() != 0.f || irradiance_value.green() != 0.f ||
 * irradiance_value.blue() != 0.f; */
...
/*     std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(gen_direct_end -
 * gen_direct_start).count(); */
...
/*     std::cout << "Computing indirect lighting (" << num_sample_sets << " sample sets, " <<
 * num_accumulation_steps << " accumulation steps)..." << std::endl; */
...
/*     std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(gen_indirect_end -
 * gen_indirect_start).count(); */
/*     std::cout << " milliseconds." << std::endl; */
...
/*     std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(gen_ambient_end -
 * gen_ambient_start).count(); */
/*     std::cout << " milliseconds.\n"; */
...
/*     { */
/*         // Run post-processing */
/*         sge::lightmap_postprocess(lightmap.width, lightmap.height, num_post_steps,
 * lightmap.basis_x_radiance); */
/*         sge::lightmap_postprocess(lightmap.width, lightmap.height, num_post_steps,
 * lightmap.basis_y_radiance); */
/*         sge::lightmap_postprocess(lightmap.width, lightmap.height, num_post_steps,
