...
struct alignas(char) Packet {
   ...
   */
  static constexpr size_t HEADER_CONTENT_LENGTH_OFFSET =
      HEADER_SEQUENCE_NUMBER_OFFSET + sizeof(SequenceNumber_t);
  ...
  static void free(Packet* packet) { sge::free(packet); }
   ...
   */
  size_t packet_size() const { return HEADER_SIZE + content_length(packet_header()); }

  /**
   * \brief Accesses the header object within this packet.
   */
  const Header_t& packet_header() const { return *reinterpret_cast<const Header_t*>(this); }

  /**
   * \brief Accesses the message inside this packet (NOT null-terminated).
   */
  const char* packet_content() const { return reinterpret_cast<const char*>(this + PACKET_CONTENT_OFFSET); }
};
}  // namespace editor_server
}  // namespace sge
